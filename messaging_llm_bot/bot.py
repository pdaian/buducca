...
class BotRunner:
    ...
    def _build_frontend_record(
        ...
        *,
        ...
        logging.debug(
            "Built frontend record backend=%s conversation=%s sender=%s source=%s sent_at=%s logged_at=%s "
            "collected_at=%s logged_collected_match=%s",
            backend,
            conversation_id,
            sender_id,
            source,
        ...
        if backend == "google_fi":
            logging.debug(
                "Google Fi frontend timestamp summary conversation=%s real_timestamp=%s timestamp_source=%s "
                "logged_at=%s collected_at=%s",
                conversation_id,
                sent_at or logged_timestamp,
                "sent_at" if sent_at else "logged_at_fallback",
                logged_timestamp,
                collected_timestamp,
            )
        return payload
