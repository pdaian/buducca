...
class BotTests(unittest.TestCase):
    ...
    def test_android_recent_dedupes_replayed_source_message_with_stable_event_id(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            cfg = BotConfig(
                android=AndroidConfig(
                    account="android",
    ...
    def test_signal_outgoing_note_to_self_is_processed_and_stored_when_unanswered(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            cfg = BotConfig(
                signal=SignalConfig(
                    account="+15550001",
    ...
    def test_signal_outgoing_self_group_message_is_processed_when_group_is_allowed(self) -> None:
        cfg = BotConfig(
            signal=SignalConfig(
                account="+15550001",
                allowed_sender_ids=["+15550002"],
                allowed_group_ids_when_sender_not_allowed=["group-123"],
            ),
            llm=LLMConfig(base_url="u", api_key="k", model="m", history_messages=2),
            runtime=RuntimeConfig(),
        )
        ...
        bot._handle_update(
            IncomingMessage(
                update_id=1,
                backend="signal",
                conversation_id="group:Family Chat|group-123",
                sender_id="+15550001",
                text="group note",
                is_outgoing=True,
            )
        ...
        self.assertEqual(bot.llm.calls, 1)
        self.assertEqual(bot.signal.sent, [("group:Family Chat|group-123", "hello")])
    ...
    def test_hourly_task_no_action_with_runner_footer_is_not_sent(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            cfg = BotConfig(
                telegram=TelegramConfig(bot_token="t"),
                llm=LLMConfig(base_url="u", api_key="k", model="m", history_messages=2),
            ...
            bot = BotRunner(cfg)
            bot.telegram = DummyTelegram()
            bot.llm = TaggedLLM("NO_ACTION", "IP: localhost | model: primary | 1.1 tok/s, 472 tok, 424.44s")
            bot._append_frontend_log(
                backend="telegram",
                direction="incoming",
                conversation_id="123",
    ...
    def test_telegram_recent_sorting_ignores_logged_at_when_sent_at_is_present(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            cfg = BotConfig(
                telegram=TelegramConfig(bot_token="t", read_only=True, store_unanswered_messages=True),
                llm=LLMConfig(base_url="u", api_key="k", model="m", history_messages=2),
            ...
            bot._append_sorted_recent_message(
                "telegram.recent",
                {
                    ...
                    "collected_at": "2026-03-10T13:00:00+00:00",
                },
            )
            bot._append_sorted_recent_message(
                "telegram.recent",
                {
                    ...
                    "collected_at": "2026-03-10T14:00:00+00:00",
                },
            )
    ...
    def test_telegram_recent_sorting_accepts_zulu_sent_at_timestamps(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            cfg = BotConfig(
                telegram=TelegramConfig(bot_token="t", read_only=True, store_unanswered_messages=True),
                llm=LLMConfig(base_url="u", api_key="k", model="m", history_messages=2),
            ...
            self.assertEqual([item["text"] for item in recent], ["older zulu", "newer zulu"])

    def test_telegram_recent_sorting_falls_back_to_logged_at_when_sent_at_is_missing(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            cfg = BotConfig(
                telegram=TelegramConfig(bot_token="t", read_only=True, store_unanswered_messages=True),
                llm=LLMConfig(base_url="u", api_key="k", model="m", history_messages=2),
    ...
    def test_telegram_recent_sorting_merges_legacy_file_before_sorting(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            legacy_entries = [
                {
                    "text": "older legacy",
                    "sent_at": "2023-02-05T14:00:06+00:00",
                    "logged_at": "2023-02-05T14:00:06+00:00",
                    "collected_at": "2026-03-15T14:05:35.548716+00:00",
                },
                {
                    "text": "middle legacy",
                    "sent_at": "2026-03-15T13:00:00+00:00",
                    "logged_at": "2026-03-15T13:00:00+00:00",
                    "collected_at": "2026-03-15T13:00:30+00:00",
                },
            ]
            (Path(td) / "telegram.messages.recent").write_text(
            ...
            bot._append_sorted_recent_message(
                "telegram.recent",
                {
                    ...
                    "sent_at": "2026-03-15T13:33:04+00:00",
                    "logged_at": "2026-03-15T13:33:04+00:00",
                    "collected_at": "2026-03-15T13:33:45.173510+00:00",
                },
            )
    ...
    def test_telegram_recent_keeps_same_text_messages_without_event_ids_when_sent_at_differs(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            cfg = BotConfig(
                telegram=TelegramConfig(bot_token="t", read_only=True, store_unanswered_messages=True),
                llm=LLMConfig(base_url="u", api_key="k", model="m", history_messages=2),
            ...
            bot._append_recent_frontend_message(
                backend="telegram",
                conversation_id="1",
                sender_id="1",
                text="same text",
    ...
    def test_telegram_recent_does_not_drop_new_event_with_same_text_as_recent_handled_query(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            cfg = BotConfig(
                telegram=TelegramConfig(bot_token="t", read_only=True, store_unanswered_messages=True),
                llm=LLMConfig(base_url="u", api_key="k", model="m", history_messages=2),
            ...
            bot._append_agenta_query_log(
                backend="telegram",
                conversation_id="1",
                sender_id="1",
                text="same text",
                reply="handled already",
                event_id="41",
            )
    ...
    def test_structured_memory_previews_are_added_at_start_of_user_prompt(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            Path(td, "assistant", "facts", "timezone.json").write_text(
                '{"statement":"Timezone is America/New_York","source":"learn"}\n',
                encoding="utf-8",
            )
    ...
    def test_telegram_updates_maintain_top_level_contact_map(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertNotIn("@alice_tg [telegram] -> 123", bot._build_system_prompt())
    ...
    def test_system_prompt_includes_skills(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertIn("Available skills", system_prompt)
            self.assertIn("Prefer the most structured skill path available", system_prompt)
            self.assertIn("echo: Echoes user text.", system_prompt)
    ...
    def test_system_prompt_includes_loaded_collector_outputs(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertIn("Generated workspace files are listed for awareness and file discovery only; they are not expanded in the prompt by default.", system_prompt)
            self.assertIn("generated workspace files (2, not expanded by default):", system_prompt)
    ...
    def test_skill_call_chain_logs_configured_max_steps(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            skills_dir = Path(td) / "skills"
            skills_dir.mkdir(parents=True)
            (skills_dir / "echo.py").write_text(
            ...
            runtime = RuntimeConfig(workspace_dir=td, skills_dir=str(skills_dir), debug=True, max_skill_chain_steps=3)
            bot = self.make_bot(runtime=runtime)
            bot.telegram = DummyTelegram()
            bot.llm = SequentialLLM(
                [
            ...
            self.assertTrue(any("Skill chain step 1/3 prompt before intermediate LLM call" in line for line in logs.output))
            self.assertTrue(any("Skill chain step 1/3 intermediate LLM response" in line for line in logs.output))
