...
class ConfigTests(unittest.TestCase):
    ...
    def test_runtime_max_skill_chain_steps_defaults_to_twelve(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            path.write_text(json.dumps(data), encoding="utf-8")
            config = load_config(path)
            self.assertEqual(config.runtime.max_skill_chain_steps, 12)

    def test_runtime_max_skill_chain_steps_must_be_positive(self) -> None:
        data = {
            "telegram": {"bot_token": "t", "long_poll_timeout_seconds": 10},
            "llm": {"base_url": "https://x", "api_key": "k", "model": "m"},
            "runtime": {"max_skill_chain_steps": 0},
        }
        with tempfile.TemporaryDirectory() as td:
            path = Path(td) / "c.json"
            path.write_text(json.dumps(data), encoding="utf-8")
